...
/******************************
 * Advanced BalanceChain Segment Logic
 ******************************/
class BalanceSegment {
  constructor({ amount, ownerKeyJwk, ownerTS }) {
    ...
    this.originalBioConst = this.computeBioConst(ownerTS, ownerTS);
    ...
    this.chainId = null;
    this.unlockIntegrityProof = null;
    this.spentProof = null;
    this.ownershipProof = null;
  }

  computeBioConst(originalTS, nowTS) {
    return INITIAL_BIO_CONSTANT + (nowTS - originalTS);
  }
  ...
  async computeUnlockProof() {
    ...
    this.unlockIntegrityProof = await sha256(msg);
  }

  async computeOwnershipProof() {
    const base = [
  ...
  async spend(senderKeyJwk, nowTS) {
    ...
    this.currentBioConst     = this.computeBioConst(this.originalOwnerTS, nowTS);
    this.ownershipChangeCount++;
    const base = [
      this.originalBioConst,
      this.previousBioConst,
    ...
    this.spentProof = await sha256(base + '|SPENT');
    await this.computeOwnershipProof();
    await this.computeUnlockProof();
  }

  async claim(newOwnerKeyJwk, nowTS) {
    this.currentOwnerKeyJwk = newOwnerKeyJwk;
    this.currentOwnerTS     = nowTS;
    this.currentBioConst    = this.computeBioConst(this.originalOwnerTS, nowTS);
    await this.computeOwnershipProof();
    await this.computeUnlockProof();
