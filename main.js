/* balance_chain_v3.js – production bundle (July 2025)                 */
/* eslint max-lines: 1200                                               */
"use strict";

/*──────────────────────── 1. CONSTANTS ───────────────────────────────*/
const KEY_HASH_SALT    = "Balance-Chain-v3";
const PBKDF2_ITERS     = 310_000;
const AES_KEY_LENGTH   = 256;             // AES-GCM 256-bit

const Protocol = Object.freeze({
  GENESIS_BIO_CONST: 1736565605,          // fixed global anchor
  ...
  HISTORY_MAX: 20                         // latest N history entries kept
});
...
const DB = Object.freeze({ NAME:"BalanceChainVaultDB", VERSION:3, STORE:"vaultStore" });
...
/* sha-256 helper */
const sha256 = data => crypto.subtle.digest(
  "SHA-256", typeof data==="string" ? enc.encode(data) : data).then(toB64);
...
const computeUnlockIntegrityProof = seg => proofHash("unlock", seg);
const computeSpentProof            = seg => proofHash("spent" , seg);
const computeOwnershipProof        = seg => proofHash("own"   , seg);
...
class VaultStorage{
  ...
  static async load(){
    ...
    return new Promise((res,rej)=>{
      ...
      rq.onsuccess=()=>{ if(!rq.result) return res(null);
        const r=rq.result;
...
const periodStrings=ts=>{const d=new Date(ts*1000);
...
class SegmentFactory{
  static createAll(owner,bioConst,ts){
    return Array.from({length:Protocol.SEGMENTS.TOTAL},(_,i)=>({
      ...
      /* lineage */
      originalOwnerKey:owner, originalOwnerTS:ts, originalBioConst:bioConst,
      previousOwnerKey:null,  previousOwnerTS:null, previousBioConst:null,
      currentOwnerKey:owner,  currentOwnerTS:ts,  currentBioConst:bioConst,

      /* unlock linkage (filled later) */
...
class VaultService{
  ...
  /* -------- internal helper to read current device rawId -------------- */
  static async _currentDeviceRawId(credIdB64){
    ...
    try{
      const cred=await navigator.credentials.get({publicKey:{
        allowCredentials:allow,challenge:rand(16),userVerification:"required"},
        mediation:"optional"});
  ...
  static async onboard(pin){
    ...
    const vault={
      ...
      authAttempts:0,lockoutUntil:null
    };
  ...
  static async unlock(pin){
    ...
    /* refresh currentBioConst values */
...
class SegmentService{
  ...
  static async unlockNextSegment(idxRef=null){
    ...
    vaultData.transactionHistory.push({
      type:"unlock", segmentIndex:locked.segmentIndex, timestamp:this._now(),
      amount:locked.amount, from:dev, to:dev
    });
  ...
  static async transferSegment(recvKey){
    ...
    seg.unlocked=false; seg.exported=true;
    ...
    vaultData.transactionHistory.push({
      ...
      amount:seg.amount, from:dev, to:recvKey
    });
...
class TokenService{
  ...
  static async claimTvmTokens(){
    ...
    const bundle=segs.map(s=>({
      segmentIndex:s.segmentIndex, spentProof:s.spentProof,
      ownershipProof:s.ownershipProof, unlockIntegrityProof:s.unlockIntegrityProof
    }));
    await ChainService.submitClaimOnChain(bundle);
    v.tvmClaimedThisYear+=avail;
    await VaultService.persist(); return bundle;
  }
}
...
const openModal=id=>{lastInvoker=document.activeElement;
...
(()=>{
  const pageSize=Limits.PAGE.DEFAULT_SIZE; let page=0;
  const txList=()=>{const v=VaultService.current;if(!v) return [];
    ...
    return v.transactionHistory.map(tx=>({
      bioIban:me,bioCatch:tx.segmentIndex,amount:tx.amount,
  ...
  window.renderTransactions=function(){
    ...
    if(!tbody) return;
    tbody.innerHTML="";
    if(!list.length){empty.style.display="";prev.style.display="none";next.style.display="none";return;}
  ...
  document.getElementById("txPrevBtn")?.addEventListener("click",()=>{if(page>0){page--;window.renderTransactions();}});
  document.getElementById("txNextBtn")?.addEventListener("click",()=>{page++;window.renderTransactions();});
})();
...
/*──────────────────────── 20. SAFE HANDLER ─────────────────────────*/
window.safeHandler=f=>Promise.resolve().then(f).catch(e=>{console.error(e);toast(e.message||"Error",true);});
...
(()=>{
  ...
  document.getElementById("catchOutBtn")?.addEventListener("click",()=>safeHandler(async()=>{
    ...
    copyToClipboard(payload); toast(`Exported ${amt} segment${amt>1?"s":""}. Payload copied.`); renderVaultUI();
  }));
  ...
  document.getElementById("catchInBtn")?.addEventListener("click",()=>safeHandler(async()=>{
    ...
    toast(`Claimed ${segs.length} segment${segs.length>1?"s":""}`); renderVaultUI();
  }));
  ...
  document.getElementById("exportFriendlyBtn")?.addEventListener("click",()=>{
    BackupService.exportFriendly(VaultService.current); localStorage.setItem("vaultBackedUp","yes");
    showBackupReminder(); toast("Friendly backup exported"); });

  document.getElementById("importVaultFileInput")?.addEventListener("change",e=>safeHandler(async()=>{
    const f=e.target.files[0]; if(!f) return;
    const txt=await f.text(); const vault=JSON.parse(txt);
...
document.getElementById("rotatePinBtn")?.addEventListener("click",()=>safeHandler(async()=>{
  ...
  const rec=await VaultStorage.load();
  const oldKey=await CryptoService.deriveKey(oldPin,rec.salt);
  const vault=await CryptoService.decrypt(oldKey,rec.iv,rec.ciphertext);
  const newSalt=rand(16);
  const newKey=await CryptoService.deriveKey(newPin,newSalt);
  const {iv,ct}=await CryptoService.encrypt(newKey,vault);
  await VaultStorage.save(iv,ct,toB64(newSalt),vault);
...
window.addEventListener("DOMContentLoaded",()=>safeHandler(async()=>{
  ...
  if(!localStorage.getItem("vaultOnboarded")){
    ...
    document.getElementById("passModalSaveBtn")?.addEventListener("click",()=>safeHandler(async()=>{
      const pin=document.getElementById("passModalInput").value; if(!pin) throw new Error("Enter passphrase");
...
window.BalanceChain=Object.freeze({
  Protocol, Limits,
  bufferToBase64:toB64, base64ToBuffer:fromB64, sha256,
